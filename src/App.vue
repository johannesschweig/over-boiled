...
const {
  bag, pot, whiteSum, currentFieldIndex,
  isExploded, drawChip, startNextRound, initBag, collectRewards, totalVictoryPoints, rubies, currentBuyingPower, masterInventory
} = useGame()
...
<template>
  <div class="min-h-screen bg-slate-900 text-slate-100 p-4 font-sans select-none overflow-x-hidden">
    <BagInventory v-if="hasCollected" :master-chips="masterInventory" :is-shop-phase="hasCollected" />
    ...
    <div class="max-w-md mx-auto space-y-6">
      ...
      <div v-else
        class="h-64 bg-slate-800 rounded-3xl flex items-center justify-center border-4 border-dashed border-slate-700">
